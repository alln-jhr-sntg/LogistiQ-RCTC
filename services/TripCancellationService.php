 ...
 *
 * release() and notifyCancelled() are deliberately separate calls, not one
 * method: both call sites wrap the trip/reservation/vehicle/driver writes
 * in one DB transaction, and a notification failure must never roll that
 * back. Callers run release() inside the transaction and notifyCancelled()
 * only after it commits.
 */
class TripCancellationService
{
    // DB writes only — call inside the caller's own transaction.
    public static function release(array $trip, string $vehicleStatus): void
    {
    ...
    // Best-effort driver notification — swallows its own failures so it
    // can never roll back a transaction it's no longer part of. Call
    // after release()'s transaction has committed.
    public static function notifyCancelled(array $trip): void
    {
        try {
